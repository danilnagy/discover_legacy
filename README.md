...
The `DOE` parameter under `algoOptions` allows you to change how the first generation of designs is generated for each job. By default, this is set to *random* to create the designs by randomly sampling from the input parameter space. However, you can also get the designs from a given generation of a previously run job. To do this you set the `DOE` parameter to a list where the first item is the name of the previously run job (this is the name of the job's folder in the main `discover` directory) and the second item is the generation you want to use. Setting this to `-1` will automatically use the last full generation created, in effect restarting the job from where it left off.
